...
for j in range(k):
  j1 = l*j
  xp = r[1]* math.cos((j1-0.5)*d)
  yp = r[1]* math.sin((j1-0.5)*d)
  xn = r[1]* math.cos((j1+0.5)*d)
  yn = r[1]* math.sin((j1+0.5)*d)
  dy = xp-xn
  dx = yn-yp
...
meshfile.write('\n%6d ascii\n' %((l+1)*k))
for j in range(l*k):
  # meshfile.write('%6d %6d %6d %6d\n' %(j,(j+1)%(l*k),l*k+(j+1)%(l*k),l*k+j))
  meshfile.write('%6d %6d %6d %6d\n' %( (j+1)%(l*k), j, l*k+j, l*k+(j+1)%(l*k)))
for j in range(k):
  meshfile.write('%6d %6d %6d %6d\n' %(l*k+l*j+1, l*k+l*j, 2*l*k+2*j, 2*l*k+2*j+1))
...
for j in range(l*k):                            
  if j % l != 0 :
    meshfile.write('%6d %6d %8d\n' %(l*k+j,l*k+(j+1)%(l*k), 100))
for j in range(k):
  meshfile.write('%6d %6d %8d\n' %(2*l*k+2*j,2*l*k+2*j+1, 10+j))
for j in range(k):
  meshfile.write('%6d %6d %8d\n' %(l*k+l*j,2*l*k+2*j, 200))
  meshfile.write('%6d %6d %8d\n' %(l*k+l*j+1,2*l*k+2*j+1, 200))
