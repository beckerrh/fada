...
DomainIntegrationLoop::DomainIntegrationLoop() : Fada::DomainIntegrationLoopInterface()
{
}

DomainIntegrationLoop::DomainIntegrationLoop(const DomainIntegrationLoop& domainintegrationloop) : Fada::DomainIntegrationLoopInterface(domainintegrationloop)
{
   assert(0);
}
...
void DomainIntegrationLoop::basicInit(const Fada::DomainSolverInterface *solver)
{
   ...
   _basicInit(_solver->getMesh(), _solver->getIntegratorManager(), _solver->getFemManager(), _solver->getFemManagerRight(), _solver->getParameterFile());
...
{
   Fada::VisitorIntegrationLoopMatrix visitor(_localglobal, _variablemanager, A);
...
/*--------------------------------------------------------------------------*/
void DomainIntegrationLoop::_basicInit(const FadaMesh::MeshInterface* mesh, const Fada::IntegratorManager* integratormanager, const Fada::FemManagerInterface* femmanager, const Fada::FemManagerInterface* femmanager_right, const Alat::ParameterFile* parameterfile)
{
...
const Alat::StringSet& DomainIntegrationLoop::getIntegrationFormulaeForTermAndDomain(FadaEnums::term t, FadaEnums::domain d) const
{
...
const DomainIntegrationLoop::IntegratorSet& DomainIntegrationLoop::getIntegratorsForTermAndDomain(FadaEnums::term t, FadaEnums::domain d, const std::string& formulaname ) const
{
  Fada::TermAndDomain term_and_domain;
  term_and_domain.first = t;
  term_and_domain.second = d;
