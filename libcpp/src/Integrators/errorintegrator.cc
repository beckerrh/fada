...
/*--------------------------------------------------------------------------*/
ErrorIntegrator::~ErrorIntegrator(){}
ErrorIntegrator::ErrorIntegrator() : Fada::Integrator(){}
ErrorIntegrator::ErrorIntegrator( const ErrorIntegrator& errorintegrator) : Fada::Integrator(errorintegrator){}
ErrorIntegrator& ErrorIntegrator::operator=( const ErrorIntegrator& errorintegrator)
{
  Fada::Integrator::operator=(errorintegrator);
  assert(0);
  return *this;
}
ErrorIntegrator* ErrorIntegrator::clone() const
{
  return new ErrorIntegrator(*this);
}
std::string ErrorIntegrator::getName() const
{
  return "ErrorIntegrator";
}

/*--------------------------------------------------------------------------*/
int ErrorIntegrator::getOrderForIntegrationFormula(int orderin, int orderout) const
{
  return 2*orderin+3;
}

/*--------------------------------------------------------------------------*/
void ErrorIntegrator::setDomainsOfTerms()
{
...
/*--------------------------------------------------------------------------*/
Fada::VariableInterface* ErrorIntegrator::constructPostProcessVariable(std::string name, const Fada::VariableManager& variablemanager) const
{
  ...
  return new Fada::Variable(name, ncomp, type);
}

/*--------------------------------------------------------------------------*/
void ErrorIntegrator::initData()
{
...
/*--------------------------------------------------------------------------*/
void ErrorIntegrator::postProcessCell(AlatEnums::residualstatus& status, double weight) const
{
  double time = getTime();
  const Alat::StringSet& outvars = getVarInfo()->getVarNamesOutput();
  for(Alat::StringSet::const_iterator p = outvars.begin(); p!=outvars.end();p++)
  {
    ...
    for(int icomp = 0; icomp < getVarInfo()->getNCompOut(*p); icomp++)
    {
      ...
      {
        std::cerr << "*** ERROR in ErrorIntegrator::postProcessCell(): unknown type " << type << "\n";
        assert(0);
        exit(1);
      }
    }
